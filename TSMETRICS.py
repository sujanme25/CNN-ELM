...
def _relative_error(actual: np.ndarray, predicted: np.ndarray, benchmark: np.ndarray = None):
    ...
    if benchmark is None or isinstance(benchmark, int):
        ...
        return _error(actual[seasonality:], predicted[seasonality:]) /               (_error(actual[seasonality:], _naive_forecasting(actual, seasonality)) + EPSILON)
...
def LM(s, o):
    """
  Legate’s and McCabe’s Index 
    
    
        s: simulated
        o: observed
    output:
        lm: Legate’s and McCabe’s Index 
    """
    s, o = filter_nan(s, o)
    sbar = np.mean(s)
    obar = np.mean(o)
    lm = 1 - (np.sum(np.abs(o-s)))/np.sum((np.abs(o-obar)))
    return lm
def RRMSE(s,o):
    s,o = filter_nan(s,o)  
    return 100.0*(np.sqrt(np.mean((s-o)**2))/np.mean(o))
def RMAE(s,o):
    s,o = filter_nan(s,o)
    sbar = np.mean(s)
    obar = np.mean(o)
    return (100/len(o))*np.sum((np.abs(s-o))/np.abs(o))
